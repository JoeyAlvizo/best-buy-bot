...
const bestBuyLoginURL = 'https://www.bestbuy.com/identity/global/signin';
...
export class BestBuy 
{
  ...
  public async purchaseProduct() 
  {
    ...
    /*try  Adding check to see if i am logged in in for loop. 
    {
      await this.loginToBestBuy(customerInformation);
    }
    catch (error) 
    {
      logger.error(error);

      if (error.message === 'Browser is considered a bot, aborting attempt. Failed on Login') throw error;
    }*/
  ...
  private async loginToBestBuy(customerInformation: CustomerInformation)
  {
      const page = await this.getPage();

      logger.info(`Navigating to ${bestBuyLoginURL}`);
      
      await page.goto(`${bestBuyLoginURL}`, { timeout: 60000 });

      logger.info(`Attempting to login with the email ${customerInformation.email}`);

      await page.type('[id="fld-e"]', customerInformation.email);
      wait(4839);
      await page.type('[id="fld-p1"]', customerInformation.password);
      wait(2234);
      await page.check('#ca-remember-me');

      logger.info(`Attempting to login with the email ${customerInformation.email}`);
      wait(1734);
      page.click('.cia-form__controls');
  ...
  private async submitUserOrder()
  {
    const page = await this.getPage();
    const customerInformation = getCustomerInformation();
    const paymentInformation = getPaymentInformation();

    logger.info('Started CC Security Code completion');
    ...
    const placeOrderButton = await page.$('.contact-card__order-button > div > button');
    ...
    if (!!placeOrderButton) 
    {
      placeOrderButton.click(); // Safty on purchasing items. Uncomment this to turn of safty!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }
  ...
  private async submitGuestOrder() 
  {
    ...
    if (!!placeOrderButton) 
    {
        await page.click('.contact-card__order-button > div > button'); // Safty on purchasing items. Uncomment this to turn of safty!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }
  ...
  {
    const page = await this.getPage();
    const logintTab = await page.$('#header-block > div.header-large > div.fullbleed-wrapper > div > nav.utility-navigation > ul > li:nth-child(1) > button > div.BtnTxt > span');
    const loginTabText = await logintTab?.innerHTML();

    //logger.info("Login tab innerText = " + loginTabText);

    logger.info("Checking if logged in");
    if (loginTabText !== 'Account') 
    {
      logger.info("Probably is logged in");
      return true;
    }
